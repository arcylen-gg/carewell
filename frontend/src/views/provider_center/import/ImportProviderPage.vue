    ...
    <div class="import-coverage-plan-description">
        ...
        <v-data-table :headers="headers" :items="import_item">
            <template slot="items" slot-scope="props">
                ...
                <td class="text-xs-center">{{ props.item.tel_number }}</td>
                <td class="text-xs-center">{{ props.item.contact_person }}</td>
                <td class="text-xs-center">{{ props.item.contact_number }}</td>
                <td class="text-xs-center">{{ props.item.email }}</td>
                <td class="text-xs-center">{{ props.item.address }}</td>
                <td class="text-xs-center">{{ props.item.payee }}</td>
            </template>
    ...
    export default 
    {
        data: () => ({
            ...
            breadcrumbs: [
                ...
                {text: 'Import Provider',disabled: true,}
            ],
            uploadPercentage: 0,
            loading_value : 0,
            headers: [
        ...
        methods : {
            ...
            exportData(){
                window.open(this.$axios.defaults.baseURL+"/api/export_file","_blank");
