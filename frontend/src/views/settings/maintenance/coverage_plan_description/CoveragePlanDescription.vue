<template>
    <div class="coverage-plan-description" v-if="route">
        <v-breadcrumbs :items="breadcrumbs" class="px-0">
            <v-icon slot="divider">chevron_right</v-icon>
        </v-breadcrumbs>
        <div class="coverage-description__menu">
            <v-text-field prepend-inner-icon="search" class="my-0" placeholder="Search Keyword..." solo flat @change="getData({status: status, keyword:filters.name})" v-model="filters.name"></v-text-field>
            <global-select-box
                v-model="filters.procedure_type_id"
                item-text="name"
                item-value="id"
                api-link="api/procedures_type"
                label="Select Type"
                :filters={is_archived:0}
                add-all
                is-load
                outline
                @input="getEmitData($event)"
		    >
            </global-select-box>
            <span></span>
            <v-btn color="success" @click="go_to_url('/settings/maintenance/import-coverage-plan-description')"><v-icon>mdi-file-import</v-icon> IMPORT PLAN DESCRIPTION</v-btn>
            <v-btn depressed color="accent"><create-plan-description/></v-btn>
        </div>
        <v-tabs color="primary" v-model="active_tab" dark icons-and-text grow>
            <v-tabs-slider></v-tabs-slider>
            <v-tab v-for="item in tab_items" :key="item.TabTitle" :disabled="loading" @click="getData({status:item.value})">
                {{ item.TabTitle }}
                <v-icon>{{ item.TabIcon }}</v-icon>
            </v-tab>
            <v-tab-item v-for="item in tab_items" :key="item.TabTitle"></v-tab-item>
        </v-tabs>
            <v-data-table 
            :headers="headers" 
            :items="list" 
            :loading="loading"
            :pagination.sync="pagination"
            :total-items="table.total"
            :rows-per-page-items="[5,15,25,35]"
            class="elevation-1"
            >
                <v-progress-linear slot="progress" color="grey" indeterminate></v-progress-linear>
                <template slot="items" slot-scope="props">
                <td class="text-xs-center">{{ props.item.id }}</td>
                <td class="text-xs-center">{{ props.item.name }}</td>
                <td class="text-xs-center">{{ props.item.procedure_type.name }}</td>
                <td>
                    <v-layout wrap>
                            <v-flex xs12 sm12 md12 class="layout justify-center align-center">
                                <view-plan-description v-if="route.accesses.show" :item="props.item">
                                <v-icon class="mx-1">mdi-eye</v-icon>
                                </view-plan-description>
                                <edit-plan-description v-if="route.accesses.edit && status == 0" :item="props.item">
                                <v-icon class="mx-1">mdi-pencil</v-icon>
                                </edit-plan-description>
                                <archive-plan-description v-if="route.accesses.archive" :item="props.item">
                                <v-icon class="mx-1">{{props.item.is_archived ? "mdi-restore" : "mdi-delete"}}</v-icon>
                                </archive-plan-description >
                            </v-flex>
                    </v-layout>
                </td>
            </template>
        </v-data-table>
    </div>
</template>

<script>
    export default {
        data: () => ({
            breadcrumbs: [
                {text: 'Settings',disabled: false,}, 
                {text: 'Maintenance',disabled: false,},
                {text: 'Coverage Plan Description',disabled: true,}
            ],
            headers: [
                {text: 'PLAN ID',value: 'plan_id'}, 
                {text: 'PLAN NAME',value: 'name'}, 
                {text: 'TYPE',value: 'type'},
                {text: 'ACTION',value: 'action'}
            ],
            tab_items: [
                {TabTitle: 'ACTIVE',TabIcon: 'mdi-account-check',value:0}, 
                {TabTitle: 'INACTIVE',TabIcon: 'mdi-account-minus',value:1}
            ],
            select_data:[],
            active_tab: 'tab-0',
            list: [],
            status:0,
            item:[],
            filters:{},
            loading:false,
            route : null,
            pagination : {},
            table: {},
        }),
        watch : {
			pagination: {
				handler () {
                    let filters = {
                        is_archived : this.status,
                        limit : this.pagination.rowsPerPage || 5,
                        page : this.pagination.page || 1,
                        name : this.filters.name || '',
                        procedure_type_id : this.filters.procedure_type_id || 'all',
                    }

					this.axiosGet(filters)
				}
        	},
		},
        methods : {
            getEmitData(data){
                this.filters.procedure_type_id = data;
                this.getData({});
                return;
            },
            getData({status = 0, keyword= ''}) {
                this.filters = {
					is_archived : this.status = status,
					limit : this.table.per_page,
					page : this.pagination.page,
                    name : keyword,
                    procedure_type_id : this.filters.procedure_type_id || 'all'
                }
                this.axiosGet(this.filters)
            },
            axiosGet(filters){
                this.loading = true
                axios.get('api/procedures'+this.generateFilterURL(filters))
                .then(response=>{
                    this.table = response.data.data
                    this.list = response.data.data.data
                })
                .catch(error=>{

                })
                .finally(() => {
                    this.loading = false
                })
            },
        },
        mounted(){
            // this.filters.procedure_type_id = 'all'

            this.route = this.get_page_access(this.pages, this.$route.name)
            EventBus.$on('archivePlanDescription', (data)=> {
                this.getData({status:this.status})
            });
            EventBus.$on('editPlanDescription', (data)=> {
                this.getData({})
            });
            EventBus.$on('createProcedures', (data)=> {
                this.getData({})
            });
             EventBus.$on('typeOfProcedures', (data)=> {
                this.getData({})
            });
        },
    }
</script>

<style lang="scss">
    @import "../../../../scss/app.scss";

    .coverage-plan-description
    {
        .v-breadcrumbs li a {
        font-size: 16px;
        }
        .coverage-description__menu
        {
            display: grid;
            grid-template-columns: repeat(5, 1fr);
            justify-content: center;
            align-self: center;
            .v-btn
            {
                height: 40px;
            }
            :nth-child(2)
            {
                .v-input__slot
                {
                    width: 100%
                    ;
                    margin-left: auto;
                }
            }
            :nth-child(3), :nth-child(4)
            {
                justify-self: end;
            }
            .v-text-field 
            {
                font-size: 15px;
                .v-input__slot {
                    border: 1px solid #919191;
                }
                padding-left: 5px;
            }
        }
        
        table
        {
            thead
            {
                tr
                {
                    th
                    {
                        text-align: center !important;
                    }
                }
            }
        }
    }
</style>

