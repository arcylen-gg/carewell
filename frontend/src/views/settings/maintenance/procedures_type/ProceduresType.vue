<template>
    <div class="procedure-type" v-if="route">
        ...
        <div class="procedure-type__menu">
            ...
            <td>&nbsp;</td>
            <td>&nbsp;</td>
            <create-procedures-type v-if="route.accesses.create">
                <v-btn depressed color="accent"><i class="fas fa-plus mr-3"></i>Create Procedure Type</v-btn> 
            </create-procedures-type>
        </div>
                ...
                <td>
                    <v-layout wrap>
                        <v-flex xs12 sm12 md12 class="layout justify-center align-center">
                            <view-procedures-type v-if="route.accesses.show" :item="props.item">
                            <v-icon class="mx-1">mdi-eye</v-icon>
                            </view-procedures-type>
                            <edit-procedures-type v-if="route.accesses.edit && status == 0" :item="props.item">
                            <v-icon class="mx-1">mdi-pencil</v-icon>
                            </edit-procedures-type>
                            <archive-procedures-type v-if="route.accesses.archive" :item="props.item">
                            <v-icon class="mx-1">{{props.item.is_archived ? "mdi-restore" : "mdi-delete"}}</v-icon>
                            </archive-procedures-type>
                        </v-flex>
                    </v-layout>
                </td>
            </template>
    ...
    export default {
        data: () => ({
            ...
            breadcrumbs: [
                ...
                {text: 'Procedure Types',disabled: true,}
            ],
            filters:{
                is_archived: 0,
            },
        ...
        EventBus.$on('archiveProceduresType', (data)=> {
            this.getData({status:this.status})
        })
        
        EventBus.$on('createProceduresType', (data)=> {
            this.getData({})
        })

        EventBus.$on('editProceduresType', (data)=> {
            this.getData({})
        })
