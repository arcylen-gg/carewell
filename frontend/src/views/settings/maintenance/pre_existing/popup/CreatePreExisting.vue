<template>
    <v-dialog v-model="dialog" scrollable persistent max-width="500px">
        <v-btn slot="activator" depressed color="accent"><i class="fas fa-plus mr-3"></i>Create Pre-Existing</v-btn>
        <v-card class="create-existing">
            <v-card-title class="title" style="background-color: #222C3C; color: #fff;">Create Pre-Existing</v-card-title>
            <v-form ref="form" @submit.prevent="save">
                <v-card-text>
                    <div class="main-holder">
                        <v-text-field color="accent" v-model="form.name" label="Name" :rules="requiredRules" outline></v-text-field>
                        <v-text-field color="accent" v-model="form.number"  type="Number" label="Number" :rules="requiredRules" outline></v-text-field>
                    </div>
                </v-card-text>
                <v-divider></v-divider>
                <v-card-actions>
        ...
        methods:{
            save(){
                if(this.$refs.form.validate()){
                    this.loading = true;
                    axios.post('api/pre-existing',this.form)
                    .then(response=>{
