<template>
    <v-dialog v-model="dialog" scrollable persistent max-width="500px">
        <v-btn slot="activator" depressed color="accent"><i class="fas fa-plus mr-3"></i>Create Payment Method</v-btn>
        <v-card class="create-payment">
            <v-card-title class="title" style="background-color: #222C3C; color: #fff;">Create Payment Method</v-card-title>
        ...
        methods:{
            save(){
                if(this.$refs.form.validate()){
                    this.loading = true;
                    axios.post('api/payment-method',this.form)
                    .then(response=>{
