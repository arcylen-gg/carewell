<template>
    <div class="procedure-type" v-if="route">
        ...
        <div class="procedure-type__menu">
            ...
            <span></span>
            <!-- <span></span> -->
            <v-btn color="success" @click="go_to_url('/settings/maintenance/import-specialization')"><v-icon>mdi-file-import</v-icon> IMPORT SPECIALIZATION</v-btn>
            <create-specialization v-if="route.accesses.create">
                <v-btn depressed color="accent"><i class="fas fa-plus mr-3"></i>Create Specialization</v-btn> 
                ...
                <td>
                    <v-layout wrap>
                        <v-flex xs12 sm12 md12 class="layout justify-center align-center">
                            <view-specialization v-if="route.accesses.show" :item="props.item">
                            <v-icon class="mx-1">mdi-eye</v-icon>
                            </view-specialization>
                            <edit-specialization v-if="route.accesses.edit && status == 0" :item="props.item">
                            <v-icon class="mx-1">mdi-pencil</v-icon>
                            </edit-specialization>
                            <archive-specialization v-if="route.accesses.archive" :item="props.item">
    ...
    export default {
        data: () => ({
            ...
            breadcrumbs: [
                ...
                {text: 'Specialization',disabled: true,}
            ],
            filters:{},
            loading: false,
            headers: [
        ...
        mounted () {
            ...
            EventBus.$on('archiveSpecialization', (data)=> {
                this.getData({status:this.status})
            })
            
            EventBus.$on('createSpecialization', (data)=> {
                this.getData({})
            })

            EventBus.$on('editSpecialization', (data)=> {
                this.getData({})
            })
        }
    }
