    ...
    <span>
       <span @click="loadDialog" style="cursor: pointer">
        ...
        <v-dialog v-model="dialog" scrollable persistent max-width="400px">
            <v-card class="archive-specialization">
                <v-card-title class="title" style="background-color: #222C3C; color: #fff;">Archive Specialization</v-card-title>
                    <v-form ref="form" @submit.prevent="save">
                        <v-card-text>
                            <div class="main-holder">
                                 You are about to {{ !item.is_archived ? 'archive' : 'restore'}} <span class="font-weight-bold black--text">"{{ item.name }}"</span> specialization. Are you sure you want to do this?
                            </div>
                        </v-card-text>        
                        <v-divider></v-divider>
                        <v-card-actions>
