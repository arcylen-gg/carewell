    ...
    <span>
       <span @click="loadDialog" style="cursor: pointer">
        ...
        <v-dialog v-model="dialog" scrollable persistent max-width="400px">
            <v-card class="archive-bank">
                <v-card-title class="title" style="background-color: #222C3C; color: #fff;">Archive Bank</v-card-title>
                    <v-form ref="form" @submit.prevent="save">
                        <v-card-text>
                            <div class="main-holder">
                                 You are about to {{ !item.is_archived ? 'archive' : 'restore'}} <span class="font-weight-bold black--text">"{{ item.name }}"</span> bank. Are you sure you want to do this?
                            </div>
                        </v-card-text>        
                        <v-divider></v-divider>
                        <v-card-actions>
...
<script>
    export default {
        ...
        save() {
            ...
            axios.put(`/api/bank/${ this.item.id }`,this.item)
            .then(response => {
