<template>
    <div class="pre-existing" v-if="route">
        ...
        <div class="search-and-create__button">
            ...
            <create-payment-term v-if="route.accesses.create"/>
        </div>
                            ...
                            <td class="text-xs-left">
                                <v-layout wrap>
                                    <v-flex xs12 sm12 md12 class="layout justify-center align-center">
                                        <view-payment-term :form="props.item"><v-icon>mdi-eye</v-icon></view-payment-term>
                                        <div v-if="!props.item.is_default">
                                            <edit-payment-term v-if="route.accesses.edit && index == 0" :form="props.item"><v-icon>mdi-pencil</v-icon></edit-payment-term>
                                            <archive-payment-term v-if="route.accesses.archive" :form="props.item">
    ...
    export default {
        data: () => ({
            breadcrumbs: [
                ...
                {text: 'Payment Term',disabled: true,}
            ],
            headers: [
                // {text: 'NO.', value: 'id'}, 
        ...
        mounted(){
            ...
            EventBus.$on('archivePaymentTerm', (data)=> {
                this.getData({status:this.status})
            });
            EventBus.$on('editPaymentTerm', (data)=> {
                this.getData({})
            });
            EventBus.$on('createPaymentTerm', (data)=> {
                this.getData({})
            });
        }
    }
