    ...
    <div class="coverage-plan" v-if="route">
        ...
        >
            ...
            <template slot="items" slot-scope="props">
                ...
                <td class="text-xs-center">{{ props.item.annual_benefit_limit | mixin_change_currency_format }}</td>
                <td class="text-xs-center">{{ props.item.maximum_benefit_limit | mixin_change_currency_format }}</td>
...
<script>
import { coveragePlanData } from '../coverage_plan_v2/js/CoveragePlanStore'
    ...
    export default {
        ...
        methods : {
           getData({status = 0,keyword = ''}){
            ...
            viewCoverage(id){
                window.open(this.$axios.defaults.baseURL+`/export/coverage_plan/print?id=${id}&user=${this.user.full_name}`,"_blank")
            },
        },
        mounted () {
