    ...
    export default {
        ...
        methods:{
            ...
            save(){
                ...
                {
                    window.open(this.$axios.defaults.baseURL+"/api/export/availment?company_id="+this.company_id,"_blank");
                    this.close();
                }
            },
            async getCompany(statusValue = 0){
                this.filters.is_archived = statusValue;
                await axios.get('api/select/company_list'+this.generateFilterURL(this.filters))
                .then(response=>{
